...
- [Problem 3 — Reproduce numbers +
  discuss](#problem-3--reproduce-numbers--discuss)
...
**eop_mm** and **fhp_mm** column. We can *guess* a value from the
corresponding category, but, if we do so, a lot of statistics, like the
mean size, will be polluted.
...
- We use **EEOP = TRUE** if **EOP category ≥ 2** from the definition in
  the article.
- For EOP size, we tolerate the missing rows, but, if the value of
  **eop_mm** and **eop_cat** are contradictory to each other, we mark
...
(FHP \> 40 mm or equivalent category 4–5) is 30.8%, similar to the
reported 34.5%.
