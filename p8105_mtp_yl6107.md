...
- For EOP size, we tolerate the missing rows, but, if value of
  **eop_mm** and **eop_cat** contradict to each other, we mark the row
  as **invalid**. FHP is also treated in the same way.
...
Overall, males show a clearly higher rate of EEOP than females across
all age groups.

The range and distribution of FHP values are similar between men and
women, although males tend to have slightly wider variability.

Across panels, there is no clear visual association between FHP and EOP
— dots are scattered rather randomly without a consistent trend.
...
(FHP \> 40 mm) is 30.8%, similar to the reported 34.5%. There are no
clear evidence to show that high FHP is more common in older subjects
