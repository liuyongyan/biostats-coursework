...
Missing **age_group** are filled, but we didn't fill the missing data in **eop_mm** and **fhp_mm** column. We can *guess* a value from the corresponding category, but, if we do so, a lot of statistics, like the mean size, will be polluted.
...
- For EOP size, we tolerate the missing rows, but, if the value of **eop_mm** and **eop_cat** are contradictory to each other, we mark the row as **invalid**. FHP is also treated in the same way.

Here is a summary about the invalid rows, which are also removed.
