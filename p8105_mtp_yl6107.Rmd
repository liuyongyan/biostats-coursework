...
over60_fhp_high <- dat %>%
  ...
  summarise(
    prop_fhp_high = mean(
      (fhp_mm > 40) | (is.na(fhp_mm) & fhp_cat >= 4),
      na.rm = TRUE
    ) * 100
  )
```

Among subjects ≥ 60 years, the proportion with high forward head posture (FHP > 40 mm) is `r round(over60_fhp_high$prop_fhp_high, 1)`%, similar to the reported 34.5%.
