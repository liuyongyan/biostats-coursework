...
Missing **age_group** are filled, but we didn't fill the missing data for the column **EOP size** and **FHP size**.

**In these two *"size"* coloumns, most of the rows with a missing *"size"* value have a corresponding *'category'* value also as 0.** It's unclear whether such a participate is in the number 0 category or that 0 just means "N/A". We should just exclude those rows from the analysis.
...
- For EOP size, we tolerate the missing rows for now, but, if the value of **eop_mm** and **eop_cat** are contradictory to each other, we mark the row as **invalid**. FHP is also treated in the same way.
...
Among subjects ≥ 60 years, the proportion with high forward head posture (FHP > 40 mm) is `r round(mean(dat$fhp_cat[dat$age_group == "60+" & !is.na(dat$fhp_mm)] >= 4) * 100, 1)`%, similar to the reported 34.5%.
